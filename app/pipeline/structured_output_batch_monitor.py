"""Monitor Vertex structured output batch job and download responses.

Step 7:
- Read submitted batch job metadata from step-6 summary.
...
SETTINGS_PATH = SCRIPT_DIR / "settings" / "structured_output_batch_monitor_settings.json"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Monitor structured output Vertex batch job and download raw responses when completed."
    )
    parser.add_argument(
        "--timeout-hours",
        type=float,
...
def main() -> None:
    ...
    source_summary_file = resolve_path_from_root(
        ...
        str(
            settings.get(
                "source_summary_file",
                f"app/output/pipeline_runs/{source_run_id}/structured_output_batch.json",
            )
        ).strip(),
    )
    ...
    if not source_summary_file.exists():
        raise FileNotFoundError(f"Step-6 summary file not found: {source_summary_file}")
    step6_summary = load_json(source_summary_file)
    step6_run_status = str(step6_summary.get("run_status", "")).strip()
    if step6_run_status != RUN_STATUS_FINISHED:
        raise ValueError(
            "Step-6 run is not finished. Expected "
            f"run_status='{RUN_STATUS_FINISHED}' in {source_summary_file}, got "
            f"'{step6_run_status or 'missing'}'."
        )

    batch_job = step6_summary.get("data", {}).get("batch_job", {})
    if not isinstance(batch_job, dict):
        raise ValueError("Invalid step-6 batch_job payload in source summary.")

    batch_job_name = str(batch_job.get("name", "")).strip()
    if not batch_job_name:
        raise ValueError("Missing batch job name in step-6 summary data.batch_job.name")

    batch_output_uri_prefix = str(
        step6_summary.get("settings", {}).get("batch_output_uri_prefix", "")
    ).strip()
    if not batch_output_uri_prefix.startswith("gs://"):
        raise ValueError("Missing or invalid batch_output_uri_prefix in step-6 summary settings.")
